...
void ptScene::compileShaders()
{
    ...
    if (!m_computeProgram->addShaderFromSourceFile(QOpenGLShader::Compute, "E:/code/c++/pathTracingTutorial/Part_0/shaders/pt_compute.glsl"))
    {
    ...
    if (!m_renderProgram->addShaderFromSourceFile(QOpenGLShader::Vertex, "E:/code/c++/pathTracingTutorial/Part_0/shaders/pt_vertex.glsl"))
    {
        qWarning() << "Failed to compile vertex shader:" << m_renderProgram->log();
        return;
    }
    if (!m_renderProgram->addShaderFromSourceFile(QOpenGLShader::Fragment, "E:/code/c++/pathTracingTutorial/Part_0/shaders/pt_fragment.glsl"))
    {
