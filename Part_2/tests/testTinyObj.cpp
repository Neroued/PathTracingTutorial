...
int main() {
    ...
    for (const auto& mat : materials) { printMaterial(mat); }
    return 0;
}
...
std::ostream& operator<<(std::ostream& os, const tinyobj::texture_option_t& texopt) {
    ...
    os << "{ type: " << textureTypeToString(texopt.type) << ", sharpness: " << texopt.sharpness
       << ", brightness: " << texopt.brightness << ", contrast: " << texopt.contrast
       << ", origin_offset: (" << texopt.origin_offset[0] << ", " << texopt.origin_offset[1] << ", "
       << texopt.origin_offset[2] << ")"
       << ", scale: (" << texopt.scale[0] << ", " << texopt.scale[1] << ", " << texopt.scale[2]
       << ")"
       << ", turbulence: (" << texopt.turbulence[0] << ", " << texopt.turbulence[1] << ", "
       << texopt.turbulence[2] << ")"
       << ", texture_resolution: " << texopt.texture_resolution
       << ", clamp: " << (texopt.clamp ? "true" : "false") << ", imfchan: " << texopt.imfchan
       << ", blendu: " << (texopt.blendu ? "true" : "false")
       << ", blendv: " << (texopt.blendv ? "true" : "false")
...
void printMaterial(const tinyobj::material_t& mat) {
    ...
    std::cout << "Ambient: (" << mat.ambient[0] << ", " << mat.ambient[1] << ", " << mat.ambient[2]
              << ")" << std::endl;
    std::cout << "Diffuse: (" << mat.diffuse[0] << ", " << mat.diffuse[1] << ", " << mat.diffuse[2]
              << ")" << std::endl;
    std::cout << "Specular: (" << mat.specular[0] << ", " << mat.specular[1] << ", "
              << mat.specular[2] << ")" << std::endl;
    std::cout << "Transmittance: (" << mat.transmittance[0] << ", " << mat.transmittance[1] << ", "
              << mat.transmittance[2] << ")" << std::endl;
    std::cout << "Emission: (" << mat.emission[0] << ", " << mat.emission[1] << ", "
    ...
    if (!mat.ambient_texname.empty())
        std::cout << "Ambient Texname: " << mat.ambient_texname << std::endl;
    if (!mat.diffuse_texname.empty())
        std::cout << "Diffuse Texname: " << mat.diffuse_texname << std::endl;
    if (!mat.specular_texname.empty())
        std::cout << "Specular Texname: " << mat.specular_texname << std::endl;
    if (!mat.specular_highlight_texname.empty())
        std::cout << "Specular Highlight Texname: " << mat.specular_highlight_texname << std::endl;
    if (!mat.bump_texname.empty()) std::cout << "Bump Texname: " << mat.bump_texname << std::endl;
    if (!mat.displacement_texname.empty())
        std::cout << "Displacement Texname: " << mat.displacement_texname << std::endl;
    if (!mat.alpha_texname.empty())
        std::cout << "Alpha Texname: " << mat.alpha_texname << std::endl;
    if (!mat.reflection_texname.empty())
        std::cout << "Reflection Texname: " << mat.reflection_texname << std::endl;
    ...
    if (!mat.roughness_texname.empty())
        std::cout << "Roughness Texname: " << mat.roughness_texname << std::endl;
    if (!mat.metallic_texname.empty())
        std::cout << "Metallic Texname: " << mat.metallic_texname << std::endl;
    if (!mat.sheen_texname.empty())
        std::cout << "Sheen Texname: " << mat.sheen_texname << std::endl;
    if (!mat.emissive_texname.empty())
        std::cout << "Emissive Texname: " << mat.emissive_texname << std::endl;
    if (!mat.normal_texname.empty())
        std::cout << "Normal Texname: " << mat.normal_texname << std::endl;
    ...
    if (!mat.unknown_parameter.empty()) {
        ...
        for (const auto& kv : mat.unknown_parameter) {
